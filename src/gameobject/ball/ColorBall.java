 ...
 */
//外面要呼叫第0張大圖的第0,0小圖
public class ColorBall extends GameObject {
    ...
    private int printHint;//0是不印 1是印叉叉
    // 扣分後印叉叉
    private Text effectHint;
    //被雷打到 
...
//色球ballNumber只能是-->1,2,6,7,8,9(色球不能是第一個)
    ...
    public ColorBall(int x, int y, int width, int height, int gameLevel, int ballNumber) {
        super(x, y, width, height);
        ballHelper = new BallHelper(width, height, ballNumber);
        isCollisioned = false;
        needPrint = true;
        this.gameLevel = gameLevel;
        name = Global.Element.BALL;//在碰撞判斷時使用
        colors = setColor(gameLevel);
        currentBallColor = ballNumber;//每顆球一開始的顏色(csv決定)
        delayCounter = new DelayCounter(4);
        printHint = 0;
        effectHint = new Text(x + width / 2, y - 10, new Color(254, 128, 136), new Font("Calabri", Font.BOLD, 36), "X");
        printThunder = false;
...
//一開始的顏色 和每次重複時重新隨機出顏色
    ...
    public void changeColor(String colorName){
        for(int i = 0; i<colors.size();i++){
            if(transformNum(colors.get(i)).equals(colorName)){
                ballHelper.changeColor(colors.get(i));
                currentBallColor = colors.get(i);
            }
        }
    }
     @Override
...
//        scoreCounter.setStr(scoreCounter.getCount());
    ...
    public void update() {
        //印叉叉
        if (printHint == 1 && delayCounter.isPause()) {
            delayCounter.reset();
            delayCounter.start();
        }
