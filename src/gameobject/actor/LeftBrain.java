 ...
 * @author Wan Zhen Lin
 */
public class LeftBrain extends MovableGameObject {
    ...
    //actor-->左腦還是右腦(左腦=1)
    //LeftBrain(印x , 印y, 印寬, 印高, 速度, 遊戲關卡 , 腦的顏色(一開始為0))
    public LeftBrain(int x, int y, int width, int height, int speed, int gameLevel, int colorNumber, int playerNum) {
        super(x, y, width, height, speed);
        actIndex = 0;
        delayCounter = new DelayCounter(Global.Actor.NORMAL_DELAY);
        brainHelper = new BrainHelper(width, height, 1, colorNumber);
        isMove = false;
    ...
    public void changeColor(String questionColorName){
        ...
        System.out.println("leftbrain"+i);
         brainHelper.setColorNumber(colors.get(i));
        currentLeftColor = colors.get(i);
    }
    ...
    //往前移動 Start
    //這裡可以調整妳要整體的前進速度上限
    public void getDisplacement() {//一開始的加速
        if (deltaY < Global.Actor.NORMAL_DELTA_Y-1) {
            deltaY += moveunit;// moveunit = 0.01111
  ...
  public void changeDelay(int i) { 
    ...
    public void changeLeftBrainPosition() {
        if (x < 250) {//確認現在所在的跑道在左道
            moveRight();//往右移
        } else if (x > 250) {
            moveLeft();
        }
